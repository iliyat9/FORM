...
// تابع تبدیل تاریخ میلادی به شمسی (تابع کامل jdate)
...
function jdate($format, $timestamp = null) {
    ...
    list($hour, $minute, $second) = explode(':', substr($date, 11, 8));
    ...
    $replacements = array(
        ...
        // ساعت و دقیقه و ثانیه (همان میلادی)
        'H' => str_pad($hour, 2, '0', STR_PAD_LEFT),
        'h' => str_pad(($hour > 12 ? $hour - 12 : $hour), 2, '0', STR_PAD_LEFT),
        'i' => str_pad($minute, 2, '0', STR_PAD_LEFT),
        's' => str_pad($second, 2, '0', STR_PAD_LEFT),
        
        // روز هفته
        'w' => date('w', $timestamp), // 0-6
        'D' => $jweekdays[date('w', $timestamp)],
        'l' => $jweekdays[date('w', $timestamp)],
    );
    ...
    return en_to_fa($result);
}

// تابع برای فرمت تاریخ در dropdown (فقط تاریخ)
function format_jalali_date($mysql_date) {
    return jdate('Y/m/d', strtotime($mysql_date));
}

// تابع برای فرمت تاریخ و زمان کامل
function format_jalali_datetime($mysql_datetime) {
    return jdate('Y/m/d H:i', strtotime($mysql_datetime));
}
...
foreach($scores as $score) {
    $chart_labels[] = $score['name_dars'];
    $chart_data[] = $score['score'];
    $chart_dates[] = format_jalali_datetime($score['date_time']);
}
...
<div class="container">
    <!-- هدر -->
    <div class="header">
        <h1><i class="fas fa-graduation-cap"></i> سیستم مدیریت نمرات - تاریخ شمسی <i class="fas fa-calendar-alt"></i></h1>
        <div class="user-info">
    ...
    <div class="date-filter-section">
            ...
            <select name="date_filter" class="date-select">
                ...
                <?php foreach($available_dates as $date): ?>
                    <option value="<?php echo $date; ?>" <?php echo $selected_date == $date ? 'selected' : ''; ?>>
                        <?php echo format_jalali_date($date); ?>
                    </option>
                <?php endforeach; ?>
            </select>
        ...
        <?php if(!empty($selected_date)): ?>
            <div class="filter-status">
                ...
                <strong><?php echo format_jalali_date($selected_date); ?></strong>
            </div>
        <?php endif; ?>
    </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <?php foreach($scores as $index => $score): 
                            ...
                            $jalali_datetime = format_jalali_datetime($score['date_time']);
                            $datetime_parts = explode(' ', $jalali_datetime);
                            $date_fa = $datetime_parts[0];
                            $time_fa = $datetime_parts[1];
                        ?>
                        <tr>
